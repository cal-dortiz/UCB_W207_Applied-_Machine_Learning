...
1. Hypertuning each model
    *  There are opportunities to hypertune each model for better performance. Hypertuning each model will lead to a higher performing ensemble model. This includes normalizing and scaling data to explore different combinations of attributes.
2. Further transformation study
    *  There are opportunities to investigate additional data transformations that may improve model performance. In addition, there are opportunities to explore the development of new attributes based on existing attributes.
